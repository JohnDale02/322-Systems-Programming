...
void* mm_malloc (size_t size) {  // take in block of size, size
  size_t reqSize;     // size to request (with Alignment and Boundary Tag)
  BlockInfo * ptrFreeBlock = NULL;   // initialize a pointer 
  size_t blockSize;    // size of block we found (at least reqSize)
  size_t precedingBlockUseTag;  // tag for preceding block above
...
/****************** Have reqSize of block to Request****************************/
  
ptrFreeBlock = searchFreeList(reqSize);  // search free list for block of requested size 

if (ptrFreeBlock == NULL){ // if no block is found
  requestMoreSpace(reqSize);
  ptrFreeBlock = searchFreeList(reqSize);
}  
//******************* Have pointer to large enough freeBlock **********************/

blockSize = SIZE(ptrFreeBlock->sizeAndTags);  // used SIZE to extract free block size
precedingBlockUseTag = ptrFreeBlock->sizeAndTags & TAG_PRECEDING_USED; // get preceding TAG data w/mask

if(blockSize - reqSize >= MIN_BLOCK_SIZE){  // if enough extra space for a block
    // we have to split the block, add split to free list with correct header and footer
  
  BlockInfo* nextBlockHeader = (BlockInfo*) UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize);  
    // get head of block we will split off; current pointer + reqSize 

  nextBlockHeader->sizeAndTags = (blockSize - reqSize) | TAG_PRECEDING_USED; 
    // updating header for new block and recording that previous block is allocated
    // now we have to do the same for footer
  
  BlockInfo* nextBlockFooter = (BlockInfo*) UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize - WORD_SIZE); 
    // blocksize - WORD_SIZE = footer of new block
  nextBlockFooter->sizeAndTags = (blockSize - reqSize) | TAG_PRECEDING_USED;
    // updating footer for new block and recording that previous block is allocated
  
  if (precedingBlockUseTag){ // if block before ptrFreeBlock is allocated
    ptrFreeBlock->sizeAndTags = reqSize | TAG_USED | TAG_PRECEDING_USED; 
      // update size, allocated status, and preceding block allocation status
  }
  else{
    ptrFreeBlock->sizeAndTags = reqSize | TAG_USED;
      // update size and allocation status
  }

  BlockInfo* splitBlock = UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize);
      // get pointer for split block -> we need to add to freelist
  
  insertFreeBlock(splitBlock);  // insert split block into freelist
}

else{   // there is not enough extra space for another block 

  BlockInfo* ptrFreeBlockFooter = UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize - WORD_SIZE);
          // pointer to footer of ptrFreeBlock

  ptrFreeBlock->sizeAndTags |= TAG_USED; // update allocation status

  if (precedingBlockUseTag){  // preceding block is used 
    ptrFreeBlock->sizeAndTags |= TAG_PRECEDING_USED;
    ptrFreeBlockFooter->sizeAndTags = reqSize | TAG_USED | TAG_PRECEDING_USED;
      // update header and footer
      }
  else{                     // preceding block not used
    ptrFreeBlockFooter->sizeAndTags = reqSize | TAG_USED;
        // update header and footer 
      }
    BlockInfo* blockInFront = UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize);
    blockInFront->sizeAndTags |= TAG_PRECEDING_USED; 
        // get block in front and set preceding block as allocated

  } 
  removeFreeBlock(ptrFreeBlock); // remove free block from free list
  return ((void*) UNSCALED_POINTER_ADD(ptrFreeBlock, WORD_SIZE));
      // returning pointer to payload
}
...
void mm_free (void *ptr) {
  ...
  blockInfo = (BlockInfo*) UNSCALED_POINTER_SUB(ptr, WORD_SIZE);
   // ptr is Payload pointer, subtract a word to get block header 
  payloadSize = SIZE(blockInfo->sizeAndTags) - WORD_SIZE;
  // use SIZE on blockInfo header to get size of block, subtract 1 word for payload size
  followingBlock = (BlockInfo*) UNSCALED_POINTER_ADD(blockInfo, payloadSize + WORD_SIZE); 
  // pointer to next block
  followingBlock->sizeAndTags &= (~TAG_PRECEDING_USED); 
  // reset TAG_PRECEDING_USED bit in next block (we are freeing prev block)
  blockInfo->sizeAndTags &= (~TAG_USED);
  // change the blocks allocation status to free 
  BlockInfo* ptrFooter = UNSCALED_POINTER_ADD(blockInfo, payloadSize);
  // get footer, we have to update it now that we are freeing block
  ptrFooter->sizeAndTags = blockInfo->sizeAndTags;
  // set block footer = header 

  insertFreeBlock(blockInfo);  // insert free block back into freelist
  coalesceFreeBlock(blockInfo);  // coalesce nearby blocks
}
...
void* mm_realloc(void* ptr, size_t size) {
  ...
  if (reallocPtr == NULL){  // if pointer is NULL, acts as mm_malloc()
    return mm_malloc(size);  // return pointer 
  }

  if (size == 0){   // size == 0 and ptr != NULL, acts as mm_free()
    mm_free(ptr); 
    return 0;  // free doesnt return pointer
  }

  reallocPtr = mm_malloc(size); 
  oldBlock = UNSCALED_POINTER_SUB(ptr, WORD_SIZE); 
  // old block header by subtracting a word from payload ptr
  oldBlockSize = SIZE(oldBlock->sizeAndTags);
  // size of old block 
  if (oldBlockSize > size){  
    size = oldBlockSize;  // needs to be at least as large as old block

    char *cptr = (char *) ptr;   // implementation of memcpy
    char *cdest = (char *) destptr;  // Typecast src and dest addrsses to (char *)
    
    for (int i=0; i<oldBlockSize; i++){
        cdest[i] = cptr[i];  // copy all data from old src to current dest
    }
  }
  mm_free(ptr);
